...
_plot=en_table(pd.read_parquet(OUT / "221_hybrid_plot_level.parquet"))
...
# Values are NOT recalculated here — they are read from outputs/25_baker_v4_intervals.json,
...
_iv=_json.loads((OUT/"25_baker_v4_intervals.json").read_text(encoding="utf-8"))
...
ax.set_ylim(gu.min()-0.06, gu.max()+0.08)
...
# values are read from outputs/222_h2_recalc.json (scripts/222_h2_recalc.py) so the figure and text cannot diverge.
_h2=_json.loads((OUT/"222_h2_recalc.json").read_text(encoding="utf-8"))
...
PANELS = [("261_artifact_merged.csv",    "(A) EMMAX without principal-component covariates\n(relatedness still controlled by K$_a$ alone)"),
          ("261_artifact_merged_pc.csv", "(B) EMMAX, declared model\n(three principal components + K$_a$)")]
...
bk = en_table(pd.read_csv(OUT/"261_canonical_blink.csv"))
...
#   our EMMAX — from 261_chr8_canonical_check.csv (6.28e-4 at 54 lines, 2.495e-3 at 52).
# Panel sizes come from the same file: 81 903 at 54 lines, 80 410 at 52.
ch8 = en_table(pd.read_csv(OUT/"261_chr8_canonical_check.csv"))
