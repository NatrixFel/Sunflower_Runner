...
pd.DataFrame(removed).to_csv(OUT/"221_removed_outliers.csv",index=False,encoding="utf-8-sig")
...
plot_long.to_parquet(OUT/"221_hybrid_plot_level.parquet")
...
vc=pd.DataFrame(rows);vc.to_csv(OUT/"222_variance_components_h2.csv",index=False,encoding="utf-8-sig")
...
bl.to_csv(OUT/"222_hybrid_blup_phenotypes.csv",encoding="utf-8-sig")
print(f"\nBLUP phenotypes: {len(bl)} hybrids × {bl.shape[1]} traits -> 222_hybrid_blup_phenotypes.csv")
