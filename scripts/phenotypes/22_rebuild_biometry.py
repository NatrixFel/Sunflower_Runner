...
Defect: 22_parse_hybrid_reps.py read each VNIIMK workbook column to the end of the
sheet, but the «Биометрия 2022/2023» sheets consist of nine blocks stacked one
under another. As a result 22_hybrid_plot_level.parquet received 159 records for
...
Input:  deposit 22_hybrid_plot_level.parquet   (productivity)
       FINAL_rebutS/02_ДАННЫЕ/Фенотипы.xlsx             (plant-level biometry)
Output: 22_hybrid_plot_level.parquet
       22_biometry_removed_outliers.csv
...
REPO = work()
SRC_PARQ = OUT / "22_hybrid_plot_level.parquet"
SRC_XLSX = REPO / "FINAL_rebutS" / "02_ДАННЫЕ" / "Фенотипы.xlsx"
OUT_PARQ = OUT / "22_hybrid_plot_level.parquet"
OUT_LOG  = OUT / "22_biometry_removed_outliers.csv"
OUT_TXT  = OUT / "fix01_biometry_summary.txt"
MOTHER = {"M1": "VK101A", "M2": "VA761A", "M3": "VK934A"}


def qc_outliers(d, thr=4.0):
    """Outlier rejection as in 22_parse_hybrid_reps.py: |z| of OLS residuals > 4."""
    x = d.copy()
...
w("")
w("before (22_hybrid_plot_level.parquet):")
...
w("")
w("after (22_hybrid_plot_level.parquet):")
