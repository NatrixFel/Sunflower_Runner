...
What it does:
  ...
  2. Compares plot-level raw data from two sources — `22_hybrid_plot_level.parquet`
  ...
  4. Writes `22_hybrid_means_corrected.csv`.
...
p1 = en_table(pd.read_parquet(OUT / "22_hybrid_plot_level.parquet"))
...
# seed yield, oil content, and 1000-seed weight — `22_hybrid_plot_level.parquet`,
...
NAME = {id(P): "field records", id(Q): "22_hybrid_plot_level.parquet"}
...
for tr in FIX:
    ...
    print(f"   {tr}: source — {'field records' if src is P else '22_hybrid_plot_level.parquet'}")
out["oil_yield"] = out["seed_yield"] * out["oil_content"] / 100.0
dst = OUT / "22_hybrid_means_corrected.csv"
