...
parser.add_argument("--phenotypes", default="22_hybrid_blup_phenotypes.csv",
...
for mode in ("bootstrap", "permutation"):
    ...
    pd.DataFrame({"min_p": minp}).to_csv(HERE / f"26_null_{mode}{tag}.csv", index=False)
...
pd.DataFrame({"chr": chf, "pos": pf, "p_dom": obs, "beta_dom": beta}).to_csv(
    HERE / f"26_obs_scan_final{tag}.csv", index=False)
print(f"\nSaved: 26_obs_scan_final{tag}.csv, 26_null_bootstrap{tag}.csv, 26_null_permutation{tag}.csv")
