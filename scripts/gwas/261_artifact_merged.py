...
def emmax_scan(y, Gi, K):
    m = ~np.isnan(y); yv = y[m]; Ks = K[np.ix_(m, m)]; G = Gi[:, m]
    ev, U = np.linalg.eigh(Ks); ev = np.maximum(ev, 1e-9)
    X = np.ones((len(yv), 1)); yr = U.T @ yv; Xr = U.T @ X
    def nll(t):
        sg, se = np.exp(t); D = sg * ev + se
        W = 1 / D; XtWX = Xr.T @ (Xr * W[:, None])
        b = (Xr.T @ (yr * W)) / XtWX[0, 0]; r = yr - Xr @ b
        return 0.5 * (np.log(D).sum() + (r ** 2 * W).sum() + np.log(XtWX[0, 0]))
    vy = np.var(yv)
...
for tag, lines in (("54_with_confectionery", lines_all),
    ...
    for tr in TRAITS:
        ...
        p, U, sw, Q, Gt, gg, df, yv, sg, se, ev, msk = emmax_scan(y, Gi, K)
        vi = np.isfinite(p)
