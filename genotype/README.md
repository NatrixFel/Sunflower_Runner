...
| `23_merge_replicates_by_rules_fast.py` | replicate VCFs → one sample (`-m none` + GT rules); var2 |
| `union/23_merge_vcfs.py` | position-wise merge of the two *delivered* files → `23_merged.vcf.gz` |
...
`../../07_архив_скриптов/`. The paper analyses `23_merge_vcfs.py`.
...
7. var2 path: `23_merge_replicates_by_rules_fast.py`, then
...
2. **Replicates of one line → one sample** —
   `23_merge_replicates_by_rules_fast.py`: `bcftools merge -m none -Ou`
...
3. **Union of the two delivered files** —
   `union/23_merge_vcfs.py`: homozygote over heterozygote at shared sites
